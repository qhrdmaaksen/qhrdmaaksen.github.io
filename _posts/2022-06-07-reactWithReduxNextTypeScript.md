...
mockResolvedValue()
-fetch fn 가 호출되었을때 결정되어야하는 값을 설정할수있게해줌
...
-mocking
먼저 mocking이 생소하신 분들을 위해서 mocking 대한 기본 개념부터 잡고 
들어가는 게 좋을 것 같습니다. mocking은 단위 테스트를 작성할 때, 해당 
코드가 의존하는 부분을 가짜(mock)로 대체하는 기법을 말합니다. 일반적으로 
테스트하려는 코드가 의존하는 부분을 직접 생성하기가 너무 부담스러운 경우 
mocking이 많이 사용
==============================================
타입스크립트 typeScript
-타입스크립트가 완전히 새로운 프로그래밍 언어가 아니라 자바스크립트에서 
확장된 언어
-자바스크립트의 기본적인 문법들과 자바스크립트 코드 작성법 if 문, for 
반복문, 객체 사용법 등을 그대로 사용
-자바스크립트의 주요 문법보다 확장된 문법을 가집니다 가장 중요한 건 정적 
타입(statically Typed)의 특징을 갖음
-대규모 프로젝트라면 소스 파일도 아주 많을 거고 많은 사람들이 동일한 코드 
베이스에서 작업할 거기 때문에 함수나 객체를 의도치 않은 방식으로 사용하는 
일이 발생하게 될 겁니다 그럼 이런 문제를 마주치게 되겠죠 경고해 주는 게 
없으니까요 이 함수는 이렇게 사용하면 안된다고 말입니다 누구도 알려주지 않죠
이 함수에는 두 개의 숫자를 사용해야 한다는 걸요 두 개의 문자열이나 다른 값 
대신에요 이럴 때 타입스크립트가 도움이 됨
-npm install -g typescript
-타입스크립트 코드가 브라우저에서 실행되지 않음,브라우저에서 타입스크립트 
코드를 실행하려고 해도 실행되지 않죠 그래서 타입스크립트를 자바스크립트 
형태로 컴파일해야 합니다 컴파일이 진행되는 동안 타입 표기는 모두 삭제될 
거예요 자바 스크립트는 타입 표기를 이해하지 못하니까요 하지만 컴파일을 
진행하면서 오류들을 찾아 알려줄 겁니다 코드 작성창에 표시된 오류 알림 외에 
추가적으로요 그러니까 컴파일 단계에서는 우리가 코드를 작성하면서 미처 
발견하지 못했던 문제점을 찾아 알려주는 거죠, 컴파일된 코드는 브라우저에서 
실행될 거고요
-npx tsc
컴파일러 사용
타입스크립트 구성 파일을 프로젝트 폴더에 추가해 타입스크립트에게
컴파일할 파일을 알려줘야함
--npx tsc with-typescript.ts 로 지정한 파일 컴파일되도록 할 수있음
-오류가 있는데도 with-typescript.js 파일이 생성된 게 보일 겁니다
기본적으로 컴파일 단계에 오류가 있더라도 컴파일은 완료됩니다 오류가 있으니 
알려주고 자바스크립트 파일도 제공하는 거죠 이 자바스크립트 파일은 
이 타입스크립트 파일을 기반으로 합니다 보시는 것처럼 거의 동일한 코드예요
다만 타입 표기가 없고 상수(const)가 변수(var)로 바뀌었죠, 기본 설정이
구 버전의 브라우저에서도 동작하는 형태로 코드를 컴파일하는 것이기 때문입니다
타입스크립트 구성 파일을 이용해 해당 설정을 바꿀 수 있지만 아직 구성 파일이 
없으니 코드를 직접 바꿔줍시다 이 부분을 제외하면 우리가 작성한 타입스크립트 
코드와 완전히 동일하죠 이제 오류를 수정할 차례입니다 이걸 수정하지 않는다면 
타입스크립트를 사용할 이유가 없죠 그럼 숫자로 변환하겠습니다 두 개의 
문자열을 숫자로요 이 상태로 저장할게요, 코드 작성창의 오류 표시가 사라졌죠
다시 컴파일해 보면 오류 없이 컴파일이 완료되고 이전과 동일하게 자바스크립트 
파일을 얻을 수 있습니다
-타입스크립트에서는 ‘number’와 ‘string’ 표기가 소문자로 시작된다는 거예요
이렇게 ‘Number’로 사용하면 안됩니다 대문자로 써도 오류가 발생하진 않지만
대문자로 쓰면 자바스크립트의 Number 객체를 가리키게 됩니다
-//type interface
/*타입 추론으로 타입스크립트는 가능한 많은 타입을 유추하려고하며 타입 
표기가 없어도 타입을 어디에 사용할지 알아내려고함*/
/*한개의 변수에 여러개의 타입을 지정
* 하나의 변수에 문자열과 숫자를 함께 저장해야 할 수도 있죠 서로 다른 형태의 
객체를 동일한 변수에 저장해야 하는 경우는 얼마든지 생길 수 있습니다
* 이런 경우에 한 개 이상의 타입을 지정할 수 있게 해주는 기능으로 유니온
(union) 타입이라는 기능*/
let course: string | number | boolean = 'react perfect guide';
course = 123;
-직접 기본(Base) 타입을 만들어 복잡한 타입을 정의해 두고 반복해서 타입을 
정의하는 대신그 타입 별칭을 사용,예를 들어, 이 객체를 처음 사용하기 전에 
타입의 형태를 타입 별칭으로 정의하는 거죠 ‘type’ 키워드를 사용해서,일반적인 
자바스크립트에는 type 키워드는 없지만 타입스크립트에는 추가되어 있음 이제 
type 키워드 뒤에 원하는 이름을 붙이면, 그게 새로운 타입의 이름이 됨.예를 
들어 ‘Person’, 이런 식으로요 그리고 등호를 넣은 다음 옆에 타입을 정의하시면 
됩니다 이건 타입스크립트에만 존재하는 기능이기 때문에 자바스크립트로 
컴파일하면 코드에서 사라지게 될 겁니다 이 등호 오른쪽에 있는 건 
자바스크립트 값(value)이 아니라 타입을 정의한 거고요 이건 우리가 만든 객체 
타입임
ex
//타입 별칭 type alias
type AliasTest = {
...
-리액트와 타입스크립트로 함수형 컴포넌트를 만들려면 React.FC 타입설정을
함수형 컴포넌트의 상수 옆에 사용합니다 그리고 홑화살괄호를 붙인 다음
그 괄호 사이에 필요한 형태의 props를 정의하는 겁니다 프로퍼티 객체의 
타입이요 컴포넌트에 직접 만든 props가 필요하다면요 그리고 컴포넌트 안에서 
직접 만든 props를 사용
...
const Todos: React.FC<{items:string[]}> =(props) => {}/*react 에서 props 는 언제나 object type*/

-물음표를 추가해 선택적인 prop으로 만들 수도 있지만 그렇게 하면 item을 갖지 
않는 경우에 대해 처리해 줘야 함

-사용 방식은 매우 간단 새롭게 만들 모든 컴포넌트에 이 타입 표기를 추가하면 
됩니다 props도 새롭게 만들어 사용한다면 이 제네릭 타입을 추가하고 홑화살괄호 안에 새로운 props의 타입을 정의하면 됨
...
============================================
react hook 정리
-예전에는 함수형 컴포넌트는 리액트 훅이 없고 컨텐츠 출력하는데에만 사용하였음
그저 프로퍼티(props)를 받고 JSX를 반환했습니다 컴포넌트 안에서 내부 state 같은 것도 변경할 수
없었습니다
-클래스형 컴포넌트의 단점은 작업량이 많다는 겁니다 함수형 컴포넌트보다 더 많은 코드를 추가해야 하죠
함수형 컴포넌트는 아주 가볍습니다 그냥 JSX를 반환하는 함수죠 클래스형 컴포넌트에는 생성자가 필요하죠
state를 초기화하려면요 컴포넌트 생명주기 메서드도 추가해야 하고요 그래서 클래스형 컴포넌트는 금방 코드가 늘어남
-2018년에 리액트 16.8 버전이 배포되면서 리액트 훅 기능이 추가되었습니다 리액트 훅 기능을 통해
이제 state 관리를 함수형 컴포넌트에서도 할 수 있게 됐죠
-react hook 은 특별한 js fn 임, React.JS 가 제공하는 함수이며 사용자 정의 함수도 만들 수 있음
-훅 함수 이름은 모두 ‘use’로 시작합니다 기본 제공되는 리액트 훅의 이름도 ‘useState()’
‘useEffect()’, ‘useRef()’ 등이죠 직접 커스텀 훅(Custom Hooks)을 만드는 경우에도
이런 식으로 이름을 붙여야 함

useState
-state를 여러 개로 분리해서 사용하세요 그게 state를 사용하는 목적이니까요, 객체나 배열 형태의 state는 여러 데이터가
동시에 변경되거나, 여러 데이터를 동시에 변경하고 싶은 경우에만 사용하세요 그게 아니라면 state를 따로 관리하는 게 좋습니다
-useState()의 목적은 함수형 컴포넌트에서 상태(state)를 관리할 수 있도록 만드는 거죠
useState() 인수에 설정할 초기 state를 넣고 호출해서요, 꼭 초기 상태를 넣어야 하는 건
아닙니다, 초기 상태로 null이나 undefined를 사용하고 싶다면 인수 없이 useState()를
호출하면 되죠 생성된 state는 리액트를 통해 관리되고 렌더링을 거쳐도 남아있습니다
useState()에서 반환되는 값은 두 개의 요소를 가진 배열이고요, 첫 번째 요소는
현재 state의 포인터고, 두 번째 요소는 상태 업데이트 함수

훅 규칙
-첫째로 훅은 반드시 함수형 컴포넌트나 우리가 만들 커스텀 훅에서만 사용해야 한다는 겁니다
두 번째는 훅을 반드시 컴포넌트의 루트에서 사용해야 한다는 겁니다 무슨 뜻이냐면
훅은 중첩 함수 같은 데서 사용할 수 없다는 거예요 여기 submitHandler가 있죠, 
안에 구현된 건 별로 없지만 어쨌든 이런 곳에서는 useState()를 사용할 수 없습니다
벌써 여기 오류 표시가 생기죠, 모던 리액트 프로젝트가 코드를 보고 훅이 사용된 위치가
리액트 함수 컴포넌트도 아니고, 또 다른 훅도 아니라는 걸 알아낸 겁니다, 그래서
사용할 수 없다는 오류 표시가 뜬 거고요 또한 조건문에서도 훅을 사용할 수 없습니다
어떤 조건에 한해 state를 생성하는 건 허용되지 않습니다, 아쉽게도 프로젝트는
이 코드가 잘못됐다는 걸 잡아내지 못하지만 사용해서는 안됩니다
반복문이나 조건문 또는 다른 함수 안에서 훅을 사용하면 안 돼요
반드시 함수형 컴포넌트나 다른 훅의 루트에서만 사용
...
-함수형 컴포넌트나 다른 훅 안에서만 사용할 수 있고, 항상 루트에서만 사용해야 합니다
useEffect()라는 이름은 이 훅이 부수 효과(Side Effect)을 관리하기 때문에 붙여졌습니다, 
HTTP 요청이 전형적인 부수 효과죠, 부수 효과란 어떤 로직이 실행되어 응용 프로그램에 예상치 못한 영향을
주는 것을 의미합니다, 예를 들어 여기서 데이터를 가져오는 동작은 
렌더링 진행 과정에서 끝나지 않고, 어쩌면 JSX 코드 범위를 벗어난 
곳까지 영향을 줄 수도 있습니다
문서 제목을 바꾼다거나 그럴 수도 있죠 그리고 일반 컴포넌트로 처리할 수 없는 이런 동작이 렌더링 과정이고요, 
useEffect()에서 중요한 건 기본적으로 useEffect()가 모든 컴포넌트의 렌더링이 끝난 뒤에 실행된다는 겁니다
이 컴포넌트가 처음으로 렌더링되고 나서, useEffect()에 전달한 함수
즉, 인자로 넘긴 함수가 실행되는 겁니다
그래서 이 함수는 컴포넌트가 리렌더링 될 때마다 실행되죠
이건 반드시 기억해야 할 내용입니다, 렌더링 이후 그리고 렌더링 될 때마다, 이 두 가지는 매우 중요해요, 렌더링 이전도, 동시도 아니고 렌더링 이후, 렌더링 될 때마다임
--useEffect 에 의존성배열인 빈 배열을 넣게되면 useEffect 안에 로직은 첫 전체 렌더링 후 한번만 실행되게됨

외부의존성
-setUserIngredients() useState 는 변수에 저장된 함수고요 이건 밖에 정의되어 있죠
하지만 이건 특별한 함수입니다, useState()가 만드니까요
리액트가 자동으로 생성하기 때문에 이 함수는 변경될 일이 없습니다, 그래서 의존성으로 치지 않음
...
firebase 에 규칙에서 엔트리를 추가해야 필터링 사용 가능
-{
  "rules": {
    ".read": "now < 1668265200000",  // 2022-11-13
    ".write": "now < 1668265200000",  // 2022-11-13
      "ingredients":{
        ".indexOn":["title"]
      }
  }
...
useCallback()
-이전에 이펙트를 실행했을 때와 값이 달라졌기 때문에 이펙트가 다시 실행됩니다
부모 컴포넌트가 리렌더링 되면서 모든 함수들이 새로 생성됐으니까요, 값이 달라졌으니
이펙트와 Search 컴포넌트를 다시 실행하는 겁니다, 이런 일을 막기 위해 리액트가 제공하는 useCallback 훅을 사용
-useCallback 에는 또 다른 인수를 넘길 수 있는데, 첫 번째 인자는 함수였고, 
두 번째 인자는 이 함수의 의존성 값을 담은 배열임
...
state 일괄 처리 및 state 업데이트에 대한 추가 정보
...
두 개념(배칭 및 새 상태를 사용할 수 있는 경우)은 훅이 있는 함수형 구성 요소와 다음이 있는 클래스 기반 구성 요소에 대해 동일한 방식으로 작동합니다. this.setState()!


useReducer()
-예전 state를 사용하거나, 다른 state의 새로운 상태를 기반으로 새롭게 state를
업데이트해야 할 경우, useState()를 사용하는 것보다 더 나은 방법이 있습니다
‘useReducer’를 대신 사용하는 거죠, 또 다른 빌트인 훅입니다
리덕스(Redux) 강의를 들었다면, 리듀서(Reducer)라는 단어에서 뭔가 추측할 수 있을 겁니다
리듀서는 여러 개의 입력을 받아 하나의 결과를 반환하는 함수죠, useReducer는 상태를 업데이트할 때
어떤 식으로 상태를 변경할 건지 정의할 수 있게 해줍니다, 그리고 해당 state를 관리해 주죠 리액트가 관리해 줄 겁니다
-리듀서는 컴포넌트가 렌더링될 때마다 리듀서 함수가 다시 생성되지 않도록 보통 컴포넌트 바깥에 정의함 컴포넌트 내부에서 일어나는 일과 관련이 적기 때문
-reducer 에는 2 개의 인자를 받는다
state 와 action 을 받으며 action 은 state 를 업데이트하는데 중요한 역할을 한다
action 은 객체 형태이며 타입을 가지고 switch 문을 사용해 액션타입에따라서 case 를 정의해 
서로 다른 코드를 수행하도록 정의 할 수 있다
-리듀서를 사용하는 게 훨씬 깔끔하죠 모든 업데이트 로직이 리듀서에 모여있으니까요, 우리는 액션만 디스패치하면 되고요
이렇게 하면 데이터가 어떻게 관리되고 있는지 파악하기가 쉽죠
그래서 state의 형태가 복잡할 땐 변경 방식을 달리할 수 있습니다, 단순히 부울 값을
...
-리액트로 코드를 편리하게 작성하려면 이런 식으로 프로젝트를 구성해서는 안 됩니다
왜냐면 자바스크립트 구문 속에 HTML 코드를 쓸 텐데 브라우저가 그 구문을 바로이해하지 못할 거 예요
그래서 리액트 프로젝트는 좀 더 복잡하고 관련 파일도 많습니다 빌드 단계를 거쳐야 해서 그렇습니다
우리가 작성하는 코드가 그대로브라우저에로 전달되지 않을 거란 의미죠
개발자들이 편한 방법으로코드를 작성하면 그 코드가 내부적으로변환된 후브라우저에 도달하는 겁니다
빌드 단계 뿐 아니라리액트 프로젝트에서는개발 서버도 필요한데요
이 개발 서버가 리액트 애플리케이션을로컬 호스팅을 합니다
예를 들어코드에 변경이 있을 때마다브라우저에 떠 있는 페이지를 업데이트 합니다
그러면 페이지를 매번직접 리로드하지 않아도 되겠죠이런 구성이 우리에게 필요한 것이기 때문에
단순히 HTML이나 자바스크립트 파일 몇 개를그냥 만들어서는 안 됩니다
대신에, 도구를 실행해서리액트 프로젝트를 만들어야 합니다, 그래야
내부적으로 구동되는 마술 같은 기능들이자동으로 프로젝트에 추가됩니다
가장 유명한 도구는Create React App 입니다구글에서 간단히
‘create-react-app’을 검색하면공식 문서 링크나Github Repository를 볼 수 있습니다
-Node.js는 자바스크립트 런타임 모듈인데요브라우저 밖에서 자바스크립트를 실행할 수 있게 해줍니다
리액트로는 Node.js를 작성하지 않습니다리액트는 Node.js 라이브러리나 프레임워크가 아니거든요
리액트는 브라우저 쪽 자바스크립트입니다하지만 Node.js는 꼭 필요해요
‘create-react-app’ 도구를 설치하거나사용하려면요그 도구로 만든 프로젝트는
내부적으로 Node.js를 계속 사용합니다자동으로 개발 서버를 리로드하거나
코드 변환을 위해서요우리가 작성한 코드를 브라우저에서구동 가능하게 변환하는 작업 말입니다
-리액트 첫 시작nodejs.org 웹사이트에서최신 Node.js를 설치해 줍니다
사이트에 들어가서 가장 최신 버전을 고르세요이곳을 클릭해서최신 버전을 설치합니다
그러면 여러분의 운영 체제에 맞는인스톨러를 다운 받을 수 있습니다
MacOS, Linux, Windows 모두 설치 가능하고요자동으로 여러분의 운영 체제를인식해서 보여줄 겁니다
그러고 나서, 인스톨러가 시키는 대로Node.js를 깔면 됩니다
여기, 제 컴퓨터는 macOS라고 보이시죠하지만 설치 과정은 윈도우와 같을 거예요그냥 디폴트 값을 선택하고 계속 진행합니다아무 것도 변경할 필요 없어요
그러면 컴퓨터에 Node.js가 설치됩니다설치가 끝나면, 이제
‘npx create-react-app’ 명령어를 실행할 준비가 된 겁니다
명령어를 실행하려면, 디폴트 터미널이나윈도우라면 명령어 창을 열어줍니다
여기에서 cd 명령어를 이용해서리액트 프로젝트를 설치하고 싶은 폴더로
이동합니다
개발 폴더, Desktop그런 폴더들요원하는 폴더로이동했다면터미널이나
명령어 창에서‘npx create-react-app’를 칩니다
그리고 끝에 이름을 지정해요‘my-first-app’ 같은, 저는 ‘react-course’로 할게요
그리고 엔터 키를 치면은‘react-course’라는 이름의 새 폴더가 생성됩니다
명령어를 실행한 그 폴더 안에요이제, y를 눌러 컨펌 합니다
‘create-react-app’ 도구를 임시로설치한다는 것을요
이제 아까 정했던 이름의폴더가 만들어집니다그리고 그 폴더 안에
새로운 리액트 프로젝트가 생길 겁니다
-react 18
이 강의 섹션은 React 17을 기반으로 녹화되었습니다. 하지만 React 18(최신 버전)에서도 전부 정상 작동됩니다!
다만 이어지는 강의 화면에 나오는 코드에 작은 차이가 있어요.
index.js에서, 영상에서 나오는 아래 코드 대신,
ReactDOM.render(<App />, document.getElementById('root'));
새로운 React 프로젝트를 생성할 때 다음과 같은 코드가 나올 텐데요,

const root = ReactDOM.createRoot(document.getElementById('root'));root.render(<App />);
보시다시피, 거의 동일한 코드입니다. 코드가 하는 역할 또한 동일하죠. React에게 루트 컴포넌트(<App/>)를 렌더링할 위치를 알려줍니다. 

-이제 각 element의 id를 선택을 합니다root로요 그러면 이 프로젝트에 포함된단일 페이지가 보입니다
Public 폴더 안에 있죠이미지 몇 개와 HTML 파일 하나가 있죠
바로 이 단일 HTML파일이이 리액트 애플리케이션을 만듭니다
리액트로 만든SPA(Single Page Application)는서버에 HTML 파일이 단 하나 뿐입니다
리액트가 그 파일상의 DOM과 화면에 표시되는 내용을제어하는 거죠
이 index.html 파일의 <body>부분을 보시면root라는 id를 가진 <div>가 있죠앞에서 보셨을 거예요바로 이 <div>가나중에
document.getElementById(‘root’) 코드를 통해작업할 <div>입니다
그러므로여기 이 코드는App.js 파일에 설정된 HTML element를
즉, 사용자 지정 <App> element를'root' id로 지정된 element에렌더링하라고리액트에게 지시하는 것입니다


-리액트 컴포넌트는jsx를 리턴하는 함수고 이를 이용해 리액트에게 지시할 수 있어요
브라우저에 어떤 내용이 보여야 하는지


-컴포넌트의 이름은 항상 대문자로 시작하자
나중에 이 함수를 HTML코드의 컴포넌트로 사용할 때 즉, 사용자 정의 HTML element로 사용할 경우
대문자로 시작해야 내장 HTML element와 구분할 수 있기 때문입니다
리액트에서 사용자 정의 컴포넌트는 대문자로 시작해야 함


-컴포넌트 재사용 시 해당 컴포넌트를 수입한 곳에서 컴포넌트를 여러개두고 기존 컴포넌트에서 내용 변경시
컴포넌트를 사용하는 모든곳에 내용이 변경되는데 이것은 
변경은 한 군데만 하고 여러 번 재사용할 수 있는 겁니다
마치 바닐라 자바스크립트에서 함수를 하나 정의하면 애플리케이션 내의 여러 곳에서
여러 번 호출해서 사용할 수 있는 것처럼요 리액트 컴포넌트도 같은 개념


-props 는 자바스크립트 객체이며 element 에 추가한 속성이 객체안에서는 키 값 쌍으로 사용됨


-만약 onClick={onClickHandler} 사용시 onClickHandler() 와 같이 소괄호를 사용하면안된다
이 함수는 실행하지 않을 거니까 괄호를 넣으면 안 됩니다 여기에서 만약 실행한다면
자바스크립트와 리액트가 코드를 검증하는 시점에 실행될 겁니다
즉 이 줄에 있는 코드가 검증될 때 함수가 실행되는 거예요 너무 이르죠
버튼을 클릭했을 때 함수를 실행하고 싶지 코드가 검증되는 순간에 실행하고 싶은 게 아니니까요
코드 검증은 브라우저에 이 내용이 렌더링되기 전에 일어나니까요
그래서 여기에서 이 함수를 지정할 때 이름만 넣으면, 리액트에게
이것은 함수이고 이 element가 클릭되었을 때 실행해야 한다고 알려주는 겁니다


-내장 컴포넌트에서는 다른 이름을 사용할 수 없으며 onClick 과 같이 내장되어있는 속성을 사용해야하고 받아야한다


-라우터 router
라우터는 특별한 도구로서 URL에서 변화를 감지하고 URL에 기초하여 화면에 보이는 내용을
변경해 줍니다 즉, 라우팅을 이용하면 사용자들을 착각하게 만들 수 있는 거죠


-router 사용시 a tag 는 사용하지 않는다 서버로 새로운 요청을 보내기때문임,
이미 리액트 애플이 실행되고있는데 다른 링크로 옮기려고 페이지에서 나갈 필요가 없기때문임
,그래서 react-router-dom 에있는 Link 컴포넌트를 사용한다, 사용하면 react-router-dom이 <a> 태그에 클릭 리스너를 추가합니다, 클릭하면
브라우저가 서버로 request 하는 것을 방지하고 옮겨 가려는 경로의 URL을 해석해서
브라우저의 URL 창을 변경합니다 그러면 서버로 request를 보내지 않고도 적절한 컴포넌트를 화면에 로드할 수 있죠
리액트와 자바스크립트 만으로요 이렇게 하면, 이미 로딩된 페이지에 머물러 있을 수 있습니다
추가 request를 보내지 않고도요 그래서 이렇게 Link를 사용하는 겁니다


-리액트, 혹은 일반적인 단일 페이지 애플리케이션(SPA)은 대부분 백엔드 API가 필요합니다
이를 통해 Request를 보내게 되죠 그러면 백엔드는 응답으로 HTML 
대신에
특정한 포맷의 데이터를 보내는데 주로 JSON 포맷입니다 즉 JSON 
포맷으로 회신을 하는 거죠
Request를 보낼 수 있는 몇몇 URL을 간단히 노출하죠 어떤 URL로 
request를 보내느냐에 따라
다른 동작이 일어날 겁니다 이게 리액트를 통해 연결하는 일종의 백엔드입니다
리액트나 Angular로 된 프론트엔드 애플리케이션은 데이터베이스에 직접 
접속해서는 안 됩니다
리액트로 데이터베이스에 직접 접속하지 않는 이유는 보안 때문입니다
리액트 애플리케이션에 작성하는 모든 코드는 페이지를 방문하는 
이들에게 노출됩니다
개발 도구를 통해 소스를 열어서 자세히 들여다보면 이 애플리케이션을 
구성하는 자바스크립트 코드를 볼 수 있죠
우리가 작성한 것과 약간 달리 보이지만 모든 코드가 여기 있는 것이나 
마찬가지죠
심지어 데이터베이스 정보도 보일 거예요 그런 위험 때문에 백엔드 API를 사용하는 겁니다
우리가 request를 보낼 백엔드 서버 말입니다 그리고 이 서버가
그 서버에 있는 데이터베이스에 접속해서 데이터베이스에 데이터를 
저장하는 거죠


-useState를 사용해 작업할 때 리액트가 state 업데이트를 즉시 처리하지 않고
작업 예약을 걸어 뒤로 미룬다는 것을 알아둬야 합니다 그래도 매우 빠른 시점에 처리하긴 하지만
즉시 처리하는 것은 아니라는 점이죠 따라서 여러분이 state를 업데이트하려 할 때
여러분의 state가 최신 상태 스냅샷에 따라 업데이트된다고 하면
이 상태 스냅샷이 최신 상태를 반영하지 못해
상태 업데이트가 불가능한 경우가 생깁니다 최신 상태 업데이트가 아직 처리되지 않았기 때문


-Next.js 및 React를 사용하여 서버 측 렌더링(SEO에 적합합니다!) 또는 풀 스택 앱을 빌드하는 방법을 알아보세요: https://acad.link/nextjs
React Native로 모바일 Android 및 iOS 앱 빌드하기: https://acad.link/react-native
풀 스택 MERN(MongoDB, ExpressJS, React, NodeJS) 웹 앱 빌드하기: https://acad.link/mern
