...
let a = "안녕하세요";
    ...
    //a의 5번째 인덱스는 아무것도 없기때문에 공백으로 출력
        console.log(a.charAt(5),a.charAt(4),a.charAt(3),a.charAt(2),a.charAt(1),a.charAt(0));
...
<body>
  ...
  <div>
    ...
    <button type="button" onclick="color_changeEx()">클릭하면 빨강색상으로 변경</button>
    <button type="button" onclick="color_changeEx2()">클릭하면 검정색상으로 변경</button>
  </div>
  <br>
  <hr>
  ...
  <div>
    ...
    <br>
    <input type="button" value="학점 입력 후 클릭!" onclick="score_msgPrint()">
    <br>
    메세지 : <input id="example_SC_msg" type="text" value="" style="width: 300px;">
  </div>
  <hr>
  <script>
  ...
  <div style="display: inline-block;">
    <form>
      <fieldset>
        ...
        <p>이 게임은 컴퓨터가 생성한 숫자를 맞추는 게임입니다. 숫자는 1부터 10사이에있어요.</p>
        숫자 : <input id="user_num" type="number" value="" size="3">
        <button type="button" onclick="game_check()"> 확인</button>
        <br>
        횟수 : <input id="opportunity_num" type="number" value="" size="5">
        힌트 : <input id="game_hint" type="text" value="">
        <input type="button" value="게임 새로 시작" onClick="window.location.href=window.location.href">
      </fieldset>
    </form>
  </div>
  <hr>
  ...
  <div>
    <button type="button" onclick="day_of_the_week()">요일 순서 팝업창 보기</button>
  </div>
  <hr>
  <!--게임 시작 팝업창 프로그램-->
  <script>
  ...
  <div>
    <button type="button" onclick="game_start()">게임 시작 알림 프로그램</button>
  </div>
  <hr>
  ...
  <script>
    ...
    for (const user_music_info in user01_like_music) {
      document.write(user_music_info + ` : ` + user01_like_music[user_music_info] + `<br>`)
    }
  </script>
</body>
