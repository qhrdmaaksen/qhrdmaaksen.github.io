...
Vmware player 는 총60개의 scsi 하드디스크 총4개의 ide 하드 디스크 장착가능

RAID 정의 및 종류
  정의 : RAID 는 여러 개의 디스크를 하나의 디스크 처럼 사용함
    비용 절감 + 신뢰성 향상 + 성능 향상의 효과를 냄
  하드웨어 RAID 
    하드웨어 제조업체에서 여러 개의 하드 디스크를 가지고 장비를 만들어서 
      그 자체를 공급함
    좀더 안정적이지만 상당히 고가
  소프트웨어 raid 
    고가의 하드웨어 raid 의 대안
  ...
  각 레이드 방식의 비교
    단순 볼륨 : 1개의 하드디스크
    Linear raid : 2개 이상 1번째부터 저장하며 디스크 추가 가능함
    raid0 : 2개 이상이여야하며 동시저장이가능하고 가장빠르다 
    raid1 : 2개 이상, 동시 저장, 결함 허용 제공, 미러링
    raid5 : 3개 이상, 결함 허용 제공, 패리티 정보 사용, 공간효율좋음
    raid6 : 4개 이상 raid 5의 개선, 결함 허용 제공, 중복패리티 정보 사용
================================================================
셸 스크립트 프로그래밍
  ...
  vi 나 gedit 으로 작성 가능
  리눅스의 많은 부분이 셸 스크립트로 작성됨

  변수의 기본
    변수를 사용하기 전에 미리 선언하지 않으며 변수에 처음 값이 할당되면서
      자동으로 변수가 생성
    모든 변수는 문자열로 취급
    변수 이름은 대소문자를 구분한다
    변수를 대입할때 = 의 좌우에는 공백이 없어야한다
