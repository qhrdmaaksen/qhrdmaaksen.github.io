...
여러 컴퓨터에서 접속시에 A유저가 로그인 한상태에서 B 유저 인덱스 접속 시
 이미 로그인한 상태로 뜨는 에러가 발생하고 있습니다. route에서 만든 
 middleware 'isNotLoggedIn'에서요! 
동시 접속이 안되는 거 같은데 왜 그럴까요 ? ㅜ 
:여러 컴퓨터로 하셨나요? 아니면 브라우저 탭을 여러 개 띄우신 건가요?
한 가지 의심되는 것은 프론트 서버인 next.js getServerSideProps에서 
쿠키를 실수로 다 같이 공유하게 코딩 실수한 것입니다.

next.js 는 서버라서 서버 세팅을 하는 셈입니다. 프론트단 최적화를 위해 
당연히 웹팩도 잘 알아야합니다.
백엔드와 프론트엔드간 api는 swagger 같은 도구로 문서화하여 전달합니다.

혹시 세션말구 토큰으로 전환해서 하면 될수도 있을까요???
지금 이 상태에서 토큰으로 전환하기에 많이 힘든가요??
:토큰으로 전환하면 될 겁니다. 전환하는 건 그리 어렵지 않습니다. 로그인 할 
때 세션 대신 토큰을 발급해서 프론트에 주고, 프론트는 요청을 보낼 때 토큰을
 헤더에 넣어서 보내고, 백엔드 라우터에는 토큰을 검사하는 미들웨어를 넣으면
  됩니다.

cors 오류가 납니다
11메가 짜리 이미지를 첨부했더니 cors 오류가 나네요. 
:nginx 설정 하셨나요? nginx에서 용량 큰 파일이 있으면 에러가 나는데 이게 
cors에러로 출력됩니다.
https://www.runit.cloud/2020/05/nginx-upload-file-size.html
사이즈를 올리시고 nginx 재부팅하시면 됩니다.
