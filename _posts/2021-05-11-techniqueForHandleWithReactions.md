...
--------------------------------------------------------
Hooks 를 사용해 컨테이너 컴포넌트 만들기
-리덕스 스토어와 연동된 컨테이너 컴포넌트를 만들때 connect 함수를 사용하는 대신 react-redux 에서 제공하는 hooks 사용 (요즘 대부분은 hooks 사용)


useSelector 로 상태 조회
-useSelector 를 사용하면 connect 를 사용하지않고도 리덕스의 상태를 조회할수있음
ex code
const result = useSelector(상태 선택 함수 )
-connect 함수 대신 useSelector 를 사용해 값을 조회하고 컴포넌트에게 props 를 넘김


useDispatch 사용해 액션 디스패치하기
- 이훅은 컴포넌트 내부에서 스토어 내장함수 dispatch 를 사용할수있게 해줌
-컨테이너 컴포넌트에서 액션을 디스패치해야한다면 useDispatch 를 사용하면됨
ex code
const dispatch = useDispatch()
dispatch({type: 'SAMPLE_ACTION'})
-useDispatch 를 사용할땐 useCallback 과 함께 사용하는 습관을 들여야함


useStore 를 사용해 리덕스 스토어 사용
-useStore hooks 를 사용하면 컴포넌트 내부에서 리덕스 스토어 객체를 직접 사용 가능
ex code
const store = useStore()
store.dispatch({type: 'SAMPLE_ACTION'})
store.getState()
-useStore 는 컴포넌트에서 정말 어쩌다가 스토어에 '직접' 접근해야하는 상황에서만 사용해야함 / 사용해야하는 상황이 흔치 않음


useActions 유틸 Hook 을 만들어서 사용하기
-useActions 는 react-redux 에 내장된 상태로 릴리즈될 계획이였으나 꼭 필요하지 않다고 판단해 제외된 훅임
-이 훅을 사용하면 여러개 액션을 사용해야하는 경우 코드를 깔끔하게 정리하여 작성 가능
--------------------------------------------------------
connect 함수와 hooks 주요 차이점
-hooks 를 사용해 컨테이너 컴포넌트를 만들때 차이점은 connect 함수를 사용해 컨테이너 컴포넌트를 만들었을 경우 해당 컨테이너 컴포넌트의 부모 컴포넌트가 리렌더링 될때 해당 컨테이너 컴포넌트의 props 가 바뀌지 않았다면 리렌더링이 자동으로 방지돼 성능이 최적화됨
-useSelector 를 사용해 리덕스 상태를 조회했을땐 최적화 작업이 자동으로 이뤄지지 않으므로 성능 최적화를 위해 React.memo 를 컨테이너 컴포넌트에 사용해줘야함
--------------------------------------------------------
리덕스 미들웨어를 통한 비동기 작업 관리
-리액트 웹 앱에서 api 서버를 연동할때 api 요청에대한 상태를 관리해야하는데 요청이 시작됐을때 로딩,요청,성공,실패 에대한 상태를 관리토록해야함

-리액트 프로젝트에서 리덕스를 사용하고있고 이런 비동기 작업을 관리해야하면 미들웨어를 사용해 효율적이고 편하게 상태를 관리할수있음

-리덕스 미들웨어는 액션을 디스패치했을때 리듀서에서 이를 처리하기에 앞서 사전에 지정된 작업들을 실행함 / 미들웨어는 액션과 리듀서사이의 중간자라고볼수있음
--액션->미들웨어->리듀서->스토어

-리듀서가 액션을 처리하기전에 미들웨어가 할수있는 작업은 여러가지가있으며 전달받은 액션을 단순히 콘솔에기록 또는 전달받은 액션 정보를 기반으로 액션을 아예 취소하거나 다른 종류의 액션을 추가로 디스패치할수있음

미들웨어 만들기
-실제 프로젝트에서 미들웨어를 직접만들어 사용할일은 많지 않음
-보통 다른 개발자가 만들어놓은 미들웨어를 사용하면됨
ex code
...
-위에있는 함수에서 파라미터로 받아오는 store 는 리덕스스토어 인스턴스를 action 은 디스패치된 액션을 가리킴
--next 파라미터는 함수 형태이며 store.dispatch 와 비슷한 역할을 하지만 next(action)을 호출하면 그다음 처리해야할 미들웨어에게 액션을 넘겨주고 만약 그다음 미들웨어가 없다면 리듀서에게 액션을 넘겨준다는 것임
-미들웨어 내부에서 store.dispatch 를 사용하면 첫 번째 미들웨어부터 다시 처리함
--만약 미들웨어에서 next 를 사용하지않으면 액션이 리듀서에 전달되지않음(액션은 무시되는것)


비동기 작업을 처리하는 미들웨어 사용
-redux-thunk 비동기 작업을 처리할때 가장많이 사용하는 미들웨어임 객체가 아닌 함수형태의 액션을 디스패치할수있게해줌
-redux-saga redux-thunk 다음으로 가장 많이 사용되는 비동기 작업 관련 미들웨어 라이브러리임 / 특정 액션이 디스패치되었을때 정해진 로직에따라 다른 액션을 디스패치시키는 규칙을 작성해 비동기 작업을 처리할수있게해줌
...
-1- modules/sample.jsx 에서 액션 타입 선언 및 액션 생성 함수 생성
-1-02- modules/sample.jsx 에서 제너레이터 함수 생성
-1-03- modules/index.js 에서 루트 사가에 sampleSaga 등록
-1-04- App.js 에서 SampleContainer 렌더링

-비동기 작업을 미들웨어로 처리할때 redux-thunk 는 일반 함수로 이뤄져있기때문에 간단명료하다는 장점이있음
--redux-saga 는 진입장벽이있을수있으나 복잡한 상황에서 효율적으로 비동기 작업을 관리할수있음
--------------------------------------------------------
코드 스플릿팅 -파일을 분리하는 작업을 코드 스플리팅이라고 함
-코드 스플릿팅은 웹 애플리케이션을 구성하는 자바스크립트 파일을 여러개의 번들로 나누어서 브라우저에게 전달하는 것을 말함
-코드 스플릿팅을 하면 초기 로딩 속도를 높일수있음
-코드 스플릿팅을 하면 브라우저에게 필요한 파일만 요청하게되고, 나머지 파일은 나중에 요청할수있음
-코드 스플릿팅을 하면 파일을 나누어서 요청하므로 브라우저에서 파일을 캐싱할수있음
-코드 스플릿팅을 하면 빌드 작업을 통해 프로젝트에서 사용되는 js 파일 안에서 불필요한 주석, 경고 메세지, 공백 등을 제거해 파일 크기를 최소화하고 브라우저에서 jsx 문법이나 다른 최신 js 문법이 원활히 실행되도록 코드의 트랜스파일 작업을할 수 있음
--프로젝트 내에 이미지와 같은 정적 파일이 있다면 해당 파일을 위한 경로도 설정됨
---이 작업은 웹팩이 담당하며 웹팩에서 별도의 설정을 하지않으면 프로젝트 사용중인 js 파일이 하나의 파일로 합쳐지고 모든 css 파일도 하나의 파일로 합쳐짐
-CRA 로 프로젝트를 빌드할 경우 최소 두개이상의 js 파일이 생성되며 CRA 의 기본 웹팩 설정에는 SplitChunks 라는 기능이적용되어 node_modules 에서 불러온 파일, 일정 크기이상의파일, 여러 파일 간에 공유된 파일을 자동으로 따로 분리시켜 캐싱의 효과를 제대로 누릴수있게해줌

-build/static 폴더에는 파일이름에 해시값이 포함되어있으며 이 값은 빌드하는 과정에서 해당 파일의 내용에 따라 생성되며 이를통해 브라우저가 새로 파일을 받아야할지 아닌지를 판단할수있음
- main 으로 시작하는 파일은 웹팩에서 자동으로 생성되는 파일이며, 이 파일에는 프로젝트에서 사용하는 모든 파일을 불러오는 코드가 들어있음
-SplitChunks 라는 웹팩 기능을 통해 자주바뀌지않는 코드들이 파일에있기때문에 캐싱의 이점을 오래 누릴 수 있음

-프로젝트에 기본 탑재된 SplitChunks 기능을 통한 코드 스플리팅은 단순히 효율적인 캐싱 효과만 있을뿐임
-코드 비동기 로딩을 통해 js 함수,객체,컴포넌트를 처음에 불러오지않고 필요한 시점에 불러와 사용할수있음
...
위와 같이 코드를 작성하면 App 컴포넌트가 렌더링될때 notify 함수가 불러와짐
--그리고 빌드하면 notify 코드가 main 파일안에 들어가게됨
ex code 파일 분리 후 코드
const onClick = () => {
  import("./notify").then(result => result.default())
}
위와 같이 코드를 작성하면 App 컴포넌트가 렌더링될때 notify 함수가 불러와지지않고, onClick 함수가 실행될때 import 함수를 통해 notify 함수를 불러옴
--그리고 빌드하면 파일을 따로 분리시켜서 저장하고 실제 함수가 필요한 지점에 파일을 불러와서 함수를 사용할 수 있음
--import 를 함수로 사용하면 Promise 를 반환함
--import 를 함수로 사용하는 문법은 표준 js 는 아니며 dynamic import 라는 문법임
--현재 웹팩에서 지원하고있고 별도의 설정없이 프로젝트에 바로 사용가능함
--이 함수를 통해 모듈을 불러올때 모듈에서ㅗ default 로 내보낸것은 result.default() 로 불러와야함


React.lazy 와 Suspense 를 통한 컴포넌트 스플리팅
-코드 스플리팅을 위해 리엑트에 내장된 기능임
-이 기능은 리액트 16.6 버전 부터 도입되었고 이전 버전에선 import 함수를 통해 불러온 다음 컴포넌트 자체를 state 에 넣는 방식으로 구현해야함
-이 기능을 사용하면 코드 스플리팅을 하기위해 state 를 따로 선언하지 않고도 간편하게 컴포넌트 코드 스플리팅을할 수 있음
-React.lazy 는 함수로 컴포넌트를 렌더링하는 시점에 비동기적으로 불러와야하는 컴포넌트를 설정할 수 있게 해줌
ex code 사용 방법
const SplitMe = React.lazy(() => import('./SplitMe'));
-Suspense 는 리액트 내장 컴포넌트로서, 코드 스플리팅된 컴포넌트를 로딩하도록 발동시키는 역할을 함
--로딩이 끝나지 않았을때 보여줄 UI 를 설정할 수 있음
ex code 
...
-코드 스플리팅을 편하게 하도록 도와주는 서드파티 라이브러리임
-장점은 서버 사이드 렌더링을 지원한다는 것이며 렌더링하기전에 필요할때 스플리팅된 파일을 미리 불러올수있는 기능도있음
-사용법은 React.lazy 와 비슷하지만 Suspense 를 사용할 필요가 없음
-미리불러오기 외에도 타임아웃, 로딩 ui 딜레이, 서버사이드렌더링호환등 다양한 기능을 제공함
-ssr 을 사용할 계획이없다면 React.lazy & Suspense 로 구현하고 계획이있다면 Loadable Components 를 사용해야함
--------------------------------------------------------
SSR 서버 사이드 렌더링
-서버사이드 렌더링은 웹 서비스의 초기 로딩속도 개선,캐싱 및 검색 엔진 최적화를 가능하게해주는 기술임
-서버사이드 렌더링을 사용하면 웹 서비스의 초기 렌더링을 사용자의 브라우저가 아닌 서버쪽에서 처리함
-사용자는 서버에서 렌더링한 html 결과물을 받아와서 그대로 사용하기때문에 초기 로동 속도도 개선되고 검색엔진 크롤링 할때도 문제가 없음
-서버 사이드 렌더링은 ui 를 서버에서 렌더링하는것을 의미함
-리액트 프로젝트는 기본적으로 클라이언트 사이드 렌더링을 하고있음 
--클라이언트 사이드 렌더링은 ui 렌더링을 브라우저에서 모두 처리하는것이며 js 를 실행해야 내가 만든 화면이 사용자에게 보여짐
-크롬 개발자 도구에서 Network 탭에 localhost 에 Response 에 root 엘리먼트가 비어있으면 해당 페이지는 처음에 빈페이지라는 뜻이고 그 이후 js 가 실행되고 리액트 컴포넌트가 렌더링되면서 보여지는것임
-서버 사이드 렌더링을 구현하면 사용자가 웹 서비스에 방문했을때 서버쪽에서 초기 렌더링을 대신해주기때문에 사용자가 html 을 전달받을때 그 내부에 렌더링된 결과물이 보임

ssr 장점
-구글,네이버,다음 등의 검색 엔진이 웹 앱의 페이지를 원활하게 수집할수있음 / 리액트로 만든 spa 는 검색 엔진 크롤러봇처럼 js 가 실행되지 않는 환경에서는 페이지가 제대로 나타나지 않기에 서버에서 클라이언트 대신 렌더링을 해주면 검색 엔진이 페이지의 내용을 제대로 수집해 갈수있음 
-구글 검색 엔진은 다른 검색엔진과 달리 검색엔진에서 js 를 실행하는 기능이 탑재되어있어서 제대로 페이지를 크롤링해갈때도있지만 모든 페이지에대해서 js 를 실행해주지는 않기때문에 웹 서비스의 검색엔진최적화를 위해서는 ssr 을 구현해주는 것이 좋음
-ssr 을 통해 초기 렌더링 성능을 개선할수있는데 ssr 이 구현되지 않은 웹 페이지에 사용자가 방문하면 js 가 로딩되고 실행때까지 사용자는 비어있는 페이지를 보며 대기해야하고 api 까지 호출해야한다면 사용자의 대기시간이 더욱 길어지게되는데 ssr 을 구현한 웹 페이지라면 js 파일 다운로드가 완료되지않은 시점에서도 html 상에 사용자가 볼수있는 컨텐츠가 있기때문에 대기 시간이 최소화되고 이로인해 사용자 경험도 향상됨

ssr 단점
-ssr 은 결국 원래 브라우저가 해야할일을 서버가 대신 처리하는것이며 서버 리소스가 사용된다는 단점이있기에 갑자기 많은 사용자가 동시에 웹 페이지에 접속하면 서버에 과부하가 발생할수있음 
--따라서 사용자가 많은 서비스라면 캐싱과 로드 밸런싱을통해서 성능을 최적화해줘야함
-ssr 을 하면 프로젝트의 구조가 좀더 복잡해지고 데이터 미리 불러오기 , 코드 스플리팅과의 호환등 등 고려해야할 사항이 많아져서 개발이 어려워질수 있음

ssr 과 코드 스플리팅 충돌
-ssr 과 code splitting 을 함께 적용하면 작업이 까다로워짐 별도의 호환 작업없이 두 기술을 함께 적용하면 페이지에 깜빡임이 발생함
